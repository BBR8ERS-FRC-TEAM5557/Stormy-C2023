...
public class MotionSegment {
    ...
    public boolean isValid() {
        if (!MathUtils.epsilonEquals(start().acc(), end().acc(), kEpsilon)) {
            // Acceleration is not constant within the segment.
            System.err.println(
                    "Segment acceleration not constant! Start acc: " + start().acc() + ", End acc: " + end().acc());
            return false;
        }
        if (Math.signum(start().vel()) * Math.signum(end().vel()) < 0.0 && !MathUtils.epsilonEquals(start().vel(), 0.0, kEpsilon)
                && !MathUtils.epsilonEquals(end().vel(), 0.0, kEpsilon)) {
