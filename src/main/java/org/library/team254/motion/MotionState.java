package org.library.team254.motion;

import static org.library.team254.motion.MotionUtil.kEpsilon;
...
public class MotionState {
    ...
    public double nextTimeAtPos(double pos) {
        if (MathUtils.epsilonEquals(pos, this.pos, kEpsilon)) {
            // Already at pos.
            return t;
        }
        if (MathUtils.epsilonEquals(acc, 0.0, kEpsilon)) {
            // Zero acceleration case.
            final double delta_pos = pos - this.pos;
            if (!MathUtils.epsilonEquals(vel, 0.0, kEpsilon) && Math.signum(delta_pos) == Math.signum(vel)) {
    ...
    public boolean equals(MotionState other, double epsilon) {
        return coincident(other, epsilon) && MathUtils.epsilonEquals(acc, other.acc, epsilon);
    }
    ...
    public boolean coincident(MotionState other, double epsilon) {
        return MathUtils.epsilonEquals(t, other.t, epsilon) && MathUtils.epsilonEquals(pos, other.pos, epsilon)
                && MathUtils.epsilonEquals(vel, other.vel, epsilon);
    }
