...
public class LinearTimeVaryingMotionProfileGoal implements IMotionProfileGoal {
    ...
    public boolean atGoalState(MotionState state) {
        return MathUtils.epsilonEquals(state.pos(), pos(state.t()), mPosTolerance) &&
               (MathUtils.epsilonEquals(state.vel(), mGoalVel, mVelTolerance)
    ...
    public boolean atGoalPos(double pos) {
        return MathUtils.epsilonEquals(pos, pos(mT), mPosTolerance);
    }
