...
public class NewAutoBalance extends CommandBase {
    ...
    PIDController newpid = new PIDController(0.05, 0, 0);
    Timer t = new Timer();
    LatchedBoolean latch = new LatchedBoolean();
    boolean passedVGate = false;
    ...
    public void execute() {
        ...
        passedVGate = Math.abs(swerve.getGyroscopePitchVelocity()) > 5.0;
        double newoutput = newpid.calculate(swerve.getGyroscopePitch(), 0);
        MathUtils.clamp(newoutput, -1.0, 1.0);
        ...
        swerve.drive(new ChassisSpeeds(newoutput, 0, 0), DriveMode.OPEN_LOOP, false, Constants.superstructure.center_of_rotation);
