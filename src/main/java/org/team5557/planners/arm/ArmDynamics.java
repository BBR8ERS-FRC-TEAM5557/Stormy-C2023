...
public class ArmDynamics {
    ...
    public ArmDynamics() {
        ...
        this.shoulder = new JointConfig(Constants.kShoulderConstants.mass,
                Constants.kShoulderConstants.length, Constants.kShoulderConstants.moi,
                Constants.kShoulderConstants.cgRadius,
                Constants.kShoulderConstants.minAngle, Constants.kShoulderConstants.maxAngle,
                Constants.kShoulderConstants.reduction,
                DCMotor.getNEO(1).withReduction(Constants.kShoulderConstants.reduction));

        this.wrist = new JointConfig(Constants.kWristConstants.mass,
                Constants.kWristConstants.length, Constants.kWristConstants.moi, Constants.kWristConstants.cgRadius,
                Constants.kWristConstants.minAngle, Constants.kWristConstants.maxAngle,
                Constants.kWristConstants.reduction,
                DCMotor.getNEO(1).withReduction(Constants.kWristConstants.reduction));
    }
    ...
    private Matrix<N2, N2> M(Vector<N2> position) {
        ...
        M.set(
                ...
                shoulder.mass * Math.pow(shoulder.cgRadius, 2.0)
                        + wrist.mass * (Math.pow(shoulder.length, 2.0) + Math.pow(wrist.cgRadius, 2.0))
                        + shoulder.moi
                        + wrist.moi
                        + 2
                                * wrist.mass
        ...
        M.set(
                ...
                wrist.mass * Math.pow(wrist.cgRadius, 2.0)
                        + wrist.moi
                        + wrist.mass * shoulder.length * wrist.cgRadius * Math.cos(position.get(1, 0)));
        M.set(
                0,
                1,
                wrist.mass * Math.pow(wrist.cgRadius, 2.0)
